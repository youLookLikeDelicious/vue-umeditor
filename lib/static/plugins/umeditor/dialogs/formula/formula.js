...
(function () {
  ...
  UM.registerWidget('formula', {
    tpl: "<div class=\"edui-formula-wrapper\">" + "<ul class=\"edui-tab-nav\"></ul>" + "<div class=\"edui-tab-content\"></div>" + "</div>",
    sourceData: {
      formula: {
        // 16个
        'common': ["{/}frac{ }{ }", "^{ }/_{ }", "x^{ }", "x_{ }", "x^{ }_{ }", "{/}bar{ }", "{/}sqrt{ }", "{/}nthroot{ }{ }", "{/}sum^{ }_{n=}", "{/}sum", "{/}log_{ }", "{/}ln{ }", "{/}int_{ }^{ }", "{/}oint_{ }^{ }", "{/}overrightarrow{ab}", "{/}overleftarrow{ab}", "^{{/}lim}_{n \\to \\infty}", '\\matrix { }{ }{ }', '{ }'],
        // 36个
        'symbol': ["+", "-", "{/}pm", "{/}times", "{/}ast", "{/}div", "/", "{/}bigtriangleup", "=", "{/}ne", "{/}approx", ">", "<", "{/}ge", "{/}le", "\\infty", "{/}cap", "{/}cup", "{/}because", "{/}therefore", "{/}subset", "{/}supset", "{/}subseteq", "{/}supseteq", "{/}nsubseteq", "{/}nsupseteq", "{/}in", "{/}ni", "{/}notin", "{/}mapsto", "{/}leftarrow", "{/}rightarrow", "{/}Leftarrow", "{/}Rightarrow", "{/}leftrightarrow", "{/}Leftrightarrow", "\\left({ }\\right)", "\\left\\{\\right\\}"],
        // 22个
        'letter': ["{/}alpha", "{/}beta", "{/}gamma", "{/}delta", "{/}varepsilon", "{/}varphi", "{/}lambda", "{/}mu", "{/}rho", "{/}sigma", "{/}omega", "{/}Gamma", "{/}Delta", "{/}Theta", "{/}Lambda", "{/}Xi", "{/}Pi", "{/}Sigma", "{/}Upsilon", "{/}Phi", "{/}Psi", "{/}Omega"]
      }
    },
    initContent: function initContent(_editor, $widget) {
      ...
      me.tabs = $.eduitab({
        selector: "#edui-formula-tab-Jpanel"
      });
      /* 初始化popup的内容 */
    ...
    initEvent: function initEvent() {
      ...
      me.root().find('.edui-tab-pane').delegate('.edui-formula-latex-item', 'click', function (evt) {
        var $item = $(this),
            latex = $item.attr('data-latex') || ''; // console.log($item)
    ...
    insertLatex: function insertLatex(latex) {
      // console.log(latex)
