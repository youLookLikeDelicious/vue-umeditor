...
(function () {
  ...
  UM.registerWidget(widgetName, {
    ...
    initController: function initController(lang) {
      var me = this,
          img = me.editor.selection.getRange().getClosedNode(),
          url;
      me.createAlignButton(["eduiVideoFloat"]); //编辑视频时初始化相关信息
     ...
     * 根据url生成视频预览
     */
    createPreviewVideo: function createPreviewVideo(url) {
      if (!url) return;
      var me = this,
          lang = me.lang,
      ...
      $("#eduiVideoPreview", me.$widget)[0].innerHTML = '<embed type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer"' + ' src="' + url + '"' + ' width="' + 420 + '"' + ' height="' + 280 + '"' + ' wmode="transparent" play="true" loop="false" menu="false" allowscriptaccess="never" allowfullscreen="true" ></embed>';
    },

    /**
     * 将单个视频信息插入编辑器中
     */
    insertSingle: function insertSingle() {
      var me = this,
     ...
     * URL转换
     */
    convert_url: function convert_url(url) {
      if (!url) return '';
      ...
      if (youkuPlay) {
        ...
        url = url.replace(/http:\/\/www\.tudou\.com\/programs\/view\/([\w\-]+)\/?/i, "http://www.tudou.com/v/$1").replace(/http:\/\/www\.youtube\.com\/watch\?v=([\w\-]+)/i, "http://www.youtube.com/v/$1").replace(/http:\/\/v\.youku\.com\/v_show\/id_([\w\-=]+)\.html/i, "http://player.youku.com/player.php/sid/$1").replace(/http:\/\/www\.56\.com\/u\d+\/v_([\w\-]+)\.html/i, "http://player.56.com/v_$1.swf").replace(/http:\/\/www.56.com\/w\d+\/play_album\-aid\-\d+_vid\-([^.]+)\.html/i, "http://player.56.com/v_$1.swf").replace(/http:\/\/v\.ku6\.com\/.+\/([^.]+)\.html/i, "http://player.ku6.com/refer/$1/v.swf").replace(/\?f=.*/, "");
      }
    ...
    updateAlignButton: function updateAlignButton(align) {
     ...
     * @param ids
     */
    createAlignButton: function createAlignButton(ids) {
      var lang = this.lang,
     ...
     * 选择切换
     */
    switchSelect: function switchSelect(selectParentId) {
     ...
     */
    findFocus: function findFocus(id, returnProperty) {
     ...
     * 末尾字符检测
     */
    endWith: function endWith(str, endStrArr) {
