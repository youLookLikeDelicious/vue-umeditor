...
(function () {
  var utils = UM.utils,
    ...
    getAllPic: function getAllPic(sel, $w, editor) {
      var me = this,
          arr = [],
          $imgs = $(sel, $w);
      $.each($imgs, function (index, node) {
        $(node).removeAttr('width').removeAttr('height'); //                if (node.width > editor.options.initialFrameWidth) {
    ...
    scale: function scale(img, max, oWidth, oHeight) {
      var width = 0,
          height = 0,
          percent,
          ow = img.width || oWidth,
    ...
    close: function close($img) {
      $img.css({
        ...
        if ($(this).parent().remove().hasClass('edui-image-upload-item')) {
          var imgs = $('.edui-image-close'); // 删除fileinput中对应的元素

          Base.tmpFileInput.splice(pos, 1); // 重新对img的id属性排序
          // 获取img元素
    ...
    // 实例化imgPreView对象
    generatePreView: function generatePreView(ele) {
    ...
    preView: function preView(url) {
      ...
      var $img = $('<img src=\'' + url + '\' class=\'edui-image-pic\' data-id=\'' + Upload.showCount + '\' />'),
          $item = $('<div class=\'edui-image-item edui-image-upload-item\'><div class=\'edui-image-close\'></div></div>').append($img); //显示图片计数+1
    ...
    callback: function callback(data) {
      var url = data;
      Upload.toggleMask('Loading....'); // 拖拽的时候，将file推入栈中
  ...
  var Upload = {
    ...
    // 追加一个form
    render: function render(sel, t) {
      var me = this;
      $(sel, me.dialog).append($(me.uploadTpl.replace(/%%/g, t))); // 当input更改时，获取imgurl

      Base.generatePreView($('.edui-image-file')[0]);
      return me;
    },
    config: function config(sel) {
      var me = this,
    ...
    getFileFormDate: function getFileFormDate() {
      // Dataform方式
     ...
     * @param url 
     */
    replaceImageUrl: function replaceImageUrl(url) {
      var imgs = $('#' + UM.umId).find('img'),
          i = 0,
          len = imgs.length,
          myEditor = Upload.editor.textarea; // 替换UM的innerHTML
    ...
    // 上传文件(图片)， 异步提交
    uploadFile: function uploadFile(pForm) {
      var inputs = [],
          data_id,
          i = 0,
          myEditor = $('#' + UM.umId),
          imgs = myEditor.find('img'),
          // 获取文本中的图片
    ...
    submit: function submit() {
      ...
      if (!this.isInit) {
        ...
        form.setAttribute('data-hook', '0'); // 先提交图片，再提交form表单,如果使用异步提交达到预览效果，则不用对提交数据进行处理
    ...
    drag: function drag() {
      ...
      if (Base.supportPreview) {
        me.dialog.find('.edui-image-content,.edui-image-file').on('drop', function (e) {
          ...
          e.preventDefault ? e.preventDefault() : e.returnValue = false; //获取文件列表
    ...
    toggleMask: function toggleMask(html) {
      var me = this;
  ...
  UM.registerWidget('image', {
    tpl: '<div class="edui-image-wrapper">' + '<ul class="edui-tab-nav">' + '<li class="edui-tab-item edui-active"><a data-context=".edui-image-local" class="edui-tab-text"><%=lang_tab_local%></a></li>' + '<li  class="edui-tab-item"><a data-context=".edui-image-JimgSearch" class="edui-tab-text"><%=lang_tab_imgSearch%></a></li>' + '</ul>' + '<div class="edui-tab-content">' + '<div class="edui-image-local edui-tab-pane edui-active">' + '<div class="edui-image-content"></div>' + '<div class="edui-image-mask"></div>' + (Base.supportPreview ? '<div class="edui-image-dragTip"><%=lang_input_dragTip%></div>' : '') + '</div>' + '<div class="edui-image-JimgSearch edui-tab-pane">' + '<div class="edui-image-searchBar">' + '<table><tr><td><input class="edui-image-searchTxt" type="text"></td>' + '<td><div class="edui-image-searchAdd"><%=lang_btn_add%></div></td></tr></table>' + '</div>' + '<div class="edui-image-searchRes"></div>' + '</div>' + '</div>' + '</div>',
