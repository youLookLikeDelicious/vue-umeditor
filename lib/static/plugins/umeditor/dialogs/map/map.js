...
var _mapTemplate = _interopRequireDefault(require("./map-template"));
...
(function () {
  ...
  UM.registerWidget(widgetName, {
     ...
     * 初始化请求API
     */
    initRequestApi: function initRequestApi() {
      var $ifr = null; //已经初始化过， 不用再次初始化
    ...
    requestMapApi: function requestMapApi(src) {
      var me = this;
      ...
      var search = new BMap.LocalSearch(city, {
        onSearchComplete: function onSearchComplete(results) {
          if (results && results.getNumPois()) {
            ...
            me.p = new BMap.Point(point.lng, point.lat);
          } else {
    ...
    getPars: function getPars(str, par) {
      ...
      ok: {
        exec: function exec(editor) {
          ...
          if (widget.root().find(".edui-map-dynamic")[0].checked) {
            ...
            url = ["about:blank;" + '?#center=' + center.lng + ',' + center.lat, '&zoom=' + zoom, '&width=' + size.width, '&height=' + size.height, '&markers=' + point.lng + ',' + point.lat].join('');
            ...
            $('.ueditor_baidumap').each(function (index, key) {
              (0, _mapTemplate["default"])(key);
            });
          } else {
            url = "http://api.map.baidu.com/staticimage?center=" + center.lng + ',' + center.lat + "&zoom=" + zoom + "&width=" + size.width + '&height=' + size.height + "&markers=" + point.lng + ',' + point.lat;
