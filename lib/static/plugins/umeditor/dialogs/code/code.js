...
(function () {
  var Base = {
    ...
    patternGt: />/gm,
    // 对特殊字符进行转义
    dealWithChar: function dealWithChar(a) {
      return a.replace(this.patternLt, '&lt;').replace(this.patternGt, '&gt;');
    },
    // 初始化，监听select 的change事件
    ...
    // 将代码插入到文档中
    insertCode: function insertCode(editor, $w) {
      var html = $w.find('textarea')[0].value || '';
      if (!html) return; // 获取代码的类型
  ...
  UM.registerWidget('code', {
    tpl: '<div class="code-type">&nbsp;&nbsp;&nbsp;&nbsp;<select name="codeType">' + '<option>apache</option>' + '<option>bash</option>' + '<option>cpp</option>' + '<option>cs</option>' + '<option>css</option>' + '<option>coffeescript</option>' + '<option>java</option>' + '<option>javascript</option>' + '<option>json</option>' + '<option>http</option>' + '<option>php</option>' + '<option>makefile</option>' + '<option>markdown</option>' + '<option>nginx</option>' + '<option>objectivec</option>' + '<option>perl</option>' + '<option>properties</option>' + '<option>python</option>' + '<option>xml</option>' + '<option>sql</option>' + '</select></div>' + '<div class="code-content"><textarea></textarea></div>',
