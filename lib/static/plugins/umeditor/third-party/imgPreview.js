...
(function () {
  ...
  if (ImgPreview.prototype.supportPreview) {
    ImgPreview.prototype.init = function () {
      var _this = this; // 添加input change的监听事件
  ...
  ImgPreview.prototype.pattern = /jpg$|jpeg$|png$|svg$|gif$|bmp$/i; // 获取图片路径和input的files对象
  ...
  if (!ImgPreview.prototype.supportPreview) {
    $(document).ready(function () {
      var iframe, div, idocument; // 创建div对象,如果不适用div作为中介，document直接追加iframe会报错

      div = document.createElement('div');
      div.id = 'preViewHidForm'; // 如果追加一个frame元素，则name属性会在ie7中存在BUG

      div.innerHTML = '<iframe name="up" style="display:none"></iframe>'; // 追加div

      document.body.appendChild(div); // 获取iframe

      iframe = document.frames['up'];
      iframe.src = '';
      iframe.target = 'blank'; // 获取iframe 
